...
export interface FileController {
  saveFile (taskId: string, name: string, content: Buffer, mimeType: string): Promise<FileObject|null>
  deleteFile (fileId: string): Promise<void>
  deleteFiles (conn: PoolClient, taskId: string): Promise<void>
  getFile (fileId: string): Promise<FileObject|null>
}
...
export function FileFactory (db: Pool, controller: DatabaseController): FileController {
  ...
  async function saveFile (taskId: string, name: string, content: Buffer, mimeType: string): Promise<FileObject|null> {
    ...
    const { rowCount } = await db.query({
      name: 'save-file',
      text: 'INSERT INTO files (id, task_id, name, mime_type, file_path) VALUES ($1, $2, $3, $4, $5)',
      values: [ id, taskId, name, mimeType, filePath ]
    })
  ...
  async function getFile (fileId: string): Promise<FileObject|null> {
    const { rows } = await db.query({
      name: 'get-file',
